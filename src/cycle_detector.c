...
bool detect_deadlock_with_algorithm(const RAG *rag, DeadlockResult *result, 
    ...
    switch (algorithm) {
        ...
        case DETECT_BANKER:
            /* Banker's algorithm - check for safe state */
            /* A state is unsafe (potential deadlock) if no safe sequence exists */
            {
                ...
                for (int r = 0; r < MAX_RESOURCES; r++) {
                    work[r] = rag->resources[r].active ? rag->resources[r].available_instances : 0;
                }
                
                /* Initialize finish = false for all processes with requests */
                for (int p = 0; p < MAX_PROCESSES; p++) {
                    finish[p] = !rag->processes[p].active;
                    if (rag->processes[p].active) {
                        bool has_request = false;
                        for (int r = 0; r < MAX_RESOURCES; r++) {
                            if (rag->request_matrix[p][r] > 0) {
                                has_request = true;
                ...
                while (changed) {
                    ...
                    for (int p = 0; p < MAX_PROCESSES; p++) {
                        ...
                        for (int r = 0; r < MAX_RESOURCES; r++) {
                            if (rag->request_matrix[p][r] > work[r]) {
                                can_satisfy = false;
                                break;
                            }
                        }
                        ...
                        if (can_satisfy) {
                            ...
                            for (int r = 0; r < MAX_RESOURCES; r++) {
                                work[r] += rag->assignment_matrix[p][r];
                            }
                            finish[p] = true;
                            changed = true;
                        }
                ...
                /* Check for unfinished processes (deadlocked) */
                for (int p = 0; p < MAX_PROCESSES; p++) {
                    if (!finish[p]) {
                        result->deadlock_detected = true;
                        result->deadlocked_processes[result->deadlocked_process_count++] = p;
